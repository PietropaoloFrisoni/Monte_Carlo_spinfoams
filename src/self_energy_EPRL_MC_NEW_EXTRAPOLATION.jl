...
length(ARGS) < 8 && error("use these arguments: DATA_SL2CFOAM_FOLDER    CUTOFF    JB    DL_MAX    IMMIRZI    STORE_FOLDER    MONTE_CARLO_ITERATIONS    NUMBER_OF_TRIALS")
...
NUMBER_OF_TRIALS = parse(Int, ARGS[8])
...
@everywhere FACE_WEIGHTS_VEC = [2 / 3, 4 / 3, 5 / 6, 7 / 6, 1.0, 1 / 3]
...
STORE_AMPLS_FOLDER = "$(STORE_FOLDER)/data/self_energy/jb_$(JB_FLOAT)/monte_carlo/Nmc_$(MONTE_CARLO_ITERATIONS)/EPRL/immirzi_$(IMMIRZI)/NEW_EXTRAPOLATION"
...
function self_energy_EPRL_MC(cutoff, jb::HalfInt, Dl::Int, Nmc::Int, vec_number_spins_configurations, spins_mc_folder::String, face_weights_vec, step=half(1))
    ...
    for pcutoff = step:step:cutoff
        ...
        @time @sync @distributed for bulk_ampls_index = 1:Nmc
            ...
            # extrapolate amplitude for each intertwiner
            v_extrapolated = zeros(size(v_Dl.a))
            v_extrapolated .= ((v_Dl.a .* v_Dlm2.a .- v_Dlm1.a .^ 2) ./ (v_Dl.a .- 2 .* v_Dlm1.a .+ v_Dlm2.a))

            # this is equivalent to:
            # for i1=1:a, i2=1:b, i3=1:c, i4=1:d, i5=1:e
            # v_extrapolated[i1,i2,i3,i4,i5] = ( (v_Dl[i1,i2,i3,i4,i5]*v_Dlm2[i1,i2,i3,i4,i5] - v_Dlm1[i1,i2,i3,i4,i5]^2 ) / (v_Dl[i1,i2,i3,i4,i5] - 2 * v_Dlm1[i1,i2,i3,i4,i5] + v_Dlm2[i1,i2,i3,i4,i5]))
            # end
            ...
            for weight_index = 1:number_of_weights
                ...
                for ib_index = 1:boundary_dim
                    bulk_ampls[bulk_ampls_index, weight_index, ib_index] = dfj^(weight) * dot(v_extrapolated[:, :, :, :, ib_index], v_extrapolated[:, :, :, :, ib_index])
                end
...
printstyled("\nstarting computation with Nmc=$(MONTE_CARLO_ITERATIONS), n_trials=$(NUMBER_OF_TRIALS), jb=$(JB), Dl_max=$(DL_MAX), Immirzi=$(IMMIRZI) up to K=$(CUTOFF)...\n"; bold=true, color=:cyan)
...
for current_trial = 1:NUMBER_OF_TRIALS
    ...
    @time ampls_tensor, stds_tensor = self_energy_EPRL_MC(CUTOFF, JB, DL_MAX, MONTE_CARLO_ITERATIONS, vec_number_spins_configurations, SPINS_MC_INDICES_FOLDER, FACE_WEIGHTS_VEC)
